...
3. SELECT MAX(name) FROM (SELECT u.*, COUNT(c.user_id) AS comments_count FROM users u LEFT JOIN comments c ON u.id = c.user_id GROUP BY u.id ORDER BY comments_count DESC) AS counts 